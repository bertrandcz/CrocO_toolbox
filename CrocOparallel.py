...
The aim of CrocOparallel class is to provide an alternative to vortex to perform CRAMPON parallalized runs.
Many of its features are similar to snowtools_git/tasks/crampon* files
'''
from CrocOrun import CrocO, CrocOpf
...
class CrocOparallel(CrocO):
    ...
    def dump_options_conf(self):
        """
        rough dump of :
        - options and conf into conf file in conf dir
        ...
        for attr, value in self.conf.__dict__.items():
    ...
    def get_spinup(self):
        ...
        if os.path.exists(self.options.spinup):
    ...
    def setup(self):
        ...
        # => CrocOrun is used to prepare all that (it can be fed with a list of dates !!)
        ...
        self.sodas = CrocOpf(self.options, self.conf)  # first because prepare the directories.
        self.escrocs = OfflinePools(self.options, self.conf)

    def run(self, cleanup=False):
        '''
        run
        '''
        for dd in self.conf.assimdates:
            #  - spawn offline
            self.escrocs.run(dd)
    ...
    def archive(self):
        ...
        # following dates
        for idd, date in enumerate(self.stopdates[1:]):
            ...
            [shutil.copyfile('/'.join([self.xpiddir, date, mbdir]) + '/PREP.nc',
                             mbdir + '/an/PREP_' + self.stopdates[idate - 1] + '.nc') for mbdir in self.mbdirs]
            [shutil.copyfile('/'.join([self.xpiddir, date, mbdir]) + '/ISBA_PROGNOSTIC.OUT.nc',
                             mbdir + '/pro/PRO_' + self.stopdates[idate - 1] + '_' + date + '.nc') for mbdir in self.mbdirs]

        for date in self.stopdates[0:-1]:
...
class OfflinePools(CrocO):
    ...
    def setup(self):
        ...
        for idate, date in enumerate(self.stopdates):
            for (mb, mbdir) in zip(self.mblist, self.mbdirs):
                ...
                self.prepare_prep(date, self.stopdates[idate - 1], mb)
        os.chdir(self.xpiddir)
    ...
    def prepare_namelist_offline(self, date, idate, mb):
        ...
        print('erlbreqv', datetime.datetime.strptime(
            self.stopdates[idate - 1] if idate - 1 > 0 else self.options.datedeb, '%Y%m%d%H'))
